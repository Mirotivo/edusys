    ...
    public class PaymentServiceTests : BaseTest
    {
        ...
        protected override void ConfigureServices(IServiceCollection services)
        {
            ...
            _mockStripePaymentGateway
                .Setup(gateway => gateway.InitiatePaymentAsync(It.IsAny<decimal>(), It.IsAny<string>(), It.IsAny<string>(), It.IsAny<string>()))
            ...
            _mockStripePaymentGateway
                .Setup(gateway => gateway.ExecutePaymentAsync(It.IsAny<string>(), It.IsAny<decimal>(), It.IsAny<string>()))
                .ReturnsAsync((string stripeCustomerId, decimal amount, string description) =>
                {
        ...
        public async Task GetSavedCardsAsync_ShouldReturnListOfCards_WhenCardsExist()
        {
            ...
            await _dbContext.UserCards.AddRangeAsync(
                new UserCard { UserId = userId, Last4 = "1111", ExpMonth = 12, ExpYear = 2025, Brand = "Visa", Type = CardType.Receiving },
                new UserCard { UserId = userId, Last4 = "2222", ExpMonth = 11, ExpYear = 2024, Brand = "Visa", Type = CardType.Paying }
            );
            await _dbContext.SaveChangesAsync();

            // Act
            var result = await _paymentService.ListSavedCardsAsync(userId);
